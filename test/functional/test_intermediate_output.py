...
from sagemaker_containers import _env, _intermediate_output, _files
import test
...
def test_intermediate_upload():
    ...
    file4 = os.path.join(dir3, 'file5.txt')
    _files.write_file(file2, 'dir1_file2!')
    _files.write_file(file3, 'dir2_file3!')
    _files.write_file(file4, 'dir1_file4!')
    ...
    _files.write_file(file_to_delete1, 'file_to_delete1!')
    os.remove(file_to_delete1)
    os.removedirs(dir_to_delete1)

    file_to_delete2_but_copy = os.path.join(intermediate_path, 'file_to_delete1.txt')
    _files.write_file(file_to_delete2_but_copy, 'file_to_delete2!')
    time.sleep(1)
    os.remove(file_to_delete2_but_copy)

    file_to_modify1 = os.path.join(dir3, 'file_to_modify1.txt')
    _files.write_file(file_to_modify1, 'dir3_file_to_modify1_1!')
    _files.write_file(file_to_modify1, 'dir3_file_to_modify1_2!')
    _files.write_file(file_to_modify1, 'dir3_file_to_modify1_3!')
    content_to_assert = 'dir3_file_to_modify1_4!'
    _files.write_file(file_to_modify1, content_to_assert)

    file_to_delete3 = os.path.join(intermediate_path, 'file_to_delete1.txt')
    _files.write_file(file_to_delete3, 'file_to_delete2!')
    os.remove(file_to_delete3)

    # the last file to be moved
    file5 = os.path.join(intermediate_path, 'file5.txt')
    _files.write_file(file5, 'file5!')
    ...
    assert not os.path.exists(file_to_delete3)
    ...
    assert not _file_exists_in_s3(
        client, os.path.join(key_prefix, os.path.relpath(file_to_delete3, intermediate_path)))
    assert not _file_exists_in_s3(
